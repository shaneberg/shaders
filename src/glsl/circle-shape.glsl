...
void main(){
    ...
    vec3 circleColor = vec3(0.0, 0.0, 0.5);
    ...
    float circleSize = 0.2 * max((cos(u_time * 2.0)), 0.05);
    float stepSize = 0.002; // * (cos(u_time) * 0.1);
    vec2 circleCenter = vec2(0.5);
    float circlePct = smoothstep(circleSize + stepSize, circleSize - stepSize, distance(st, circleCenter));
